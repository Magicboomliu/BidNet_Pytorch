...
if __name__=="__main__":
    ...
    transmision_L1_meter = AverageMeter()
    ...
    pretrained_net = DSSMMD(in_channels=3,dehaze_switch=True)
    ...
    for i, sample_batched in enumerate(test_loader):
        ...
        with torch.no_grad():
            disparity_pyramid,pred_trans,pred_airlight = pretrained_net(haze_left,haze_right)
        ...
        # plt.subplot(2,2,2)
        # plt.imshow(haze_left.squeeze(0).permute(1,2,0).cpu().numpy())
        # plt.subplot(2,2,3)
        # plt.imshow(pred_trans.squeeze(0).squeeze(0).cpu().numpy())
        # plt.subplot(2,2,4)
        # plt.imshow(dehaze_image.squeeze(0).permute(1,2,0).cpu().numpy())
        # plt.show()
        # break
